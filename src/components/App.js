...
function App() {
  const [pens, setPensState] = useState([
    { title: "cow", animals: [1] },
    { title: "pig", animals: [2] },
  ]);
  ...
  const [animals, setAnimalsState] = useState([
    ...
    },
    {
      ...
      name: "david",
      image:
        "https://external-content.duckduckgo.com/iu/?u=https%3A%2F%2Fthenypost.files.wordpress.com%2F2018%2F04%2F180427-pig-brains-outside-body-feature.jpg%3Fquality%3D90%26strip%3Dall%26w%3D1200&f=1&nofb=1",
    },
  ]);

  const slaughter = (animal) => {
    let foundPen = pens.find((pen) => pen.title === animal.species);
    let newAnimals = foundPen.animals.filter(
  ...
  let currentBiggestId =
    animals.length < 1 ? 1 : animals[animals.length - 1].id;
  ...
  return (
    <div className="App">
      <Container className="farm">
        ...
        <Row>
          <Col>
            <Pens slaughter={slaughter} pens={pens} animals={animals} />
          </Col>
        </Row>
      </Container>
    </div>
